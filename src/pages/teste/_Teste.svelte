<script lang="ts">
  ...
  let campo_end: string;
  let ano: string = "2024";
  let doenca: string = "Dengue";

  let isCarregando = false;
  $: isformValid = file && campo_end;
  ...
  async function handleSubmit(event: Event) {
    ...
    isCarregando = false;
    window.location.href = "/maps/custom";
  ...
  <div class="p-4">
    ...
    >
      ...
      <input
        ...
        bind:value={file}
        class="border {!file ? 'border-red-300' : 'border-green-300'}"
      />
      ...
      {/if}
      <!-- <input type="campo_end" /> -->
      <button
        class="bg-green-300 p-1 rounded w-full mt-3 disabled:bg-secondary disabled:opacity-70"
...
{#if erros}
  <div class="flex justify-center items-center">
    <div class="p-4">
      <div class="rounded p-2 m-2 bg-red-200">
        <p>{erros}</p>
      </div>
    </div>
  </div>
        ...
        <tr>
          {#each Object.keys(parsedData[0]) as header}
            <th
              ...
              on:click={() => (campo_end = header)}>{header}</th
            >
          {/each}
        </tr>
      </thead>
